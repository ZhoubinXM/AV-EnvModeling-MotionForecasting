...
from train_eval.utils import get_name, get_angle, rotate, larger, get_pad_vector, assert_
from train_eval.utils import get_dis, get_subdivide_points
...
def get_sub_map(args: Dict,
    ...
    if not args['use_map']:
        ...
        if args['semantic_lane']:
            ...
            local_lane_centerlines = [
                am.get_lane_segment_centerline(lane_id, city_name)
            ...
            if args['visualize']:
                angle = mapping['angle']
                vis_lanes = [
                    am.get_lane_segment_polygon(lane_id, city_name)[:, :2]
                    for lane_id in lane_ids
                ]
                t = []
                for each in vis_lanes:
                    for point in each:
                        point[0], point[1] = rotate(point[0] - x, point[1] - y,
                                                    angle)
                    num = len(each) // 2
            ...
            polygons = am.find_local_lane_centerlines(
                x,
                y,
                city_name,
                query_search_range_manhattan=args['max_distance'])
        ...
        for index_polygon, polygon in enumerate(polygons):
            ...
            if args['semantic_lane']:
                ...
                lane_segment = am.city_lane_centerlines_dict[city_name][
                    lane_id]
            assert_(len(polygon) >= 2)
            for i, point in enumerate(polygon):
                if i > 0:
                    vector = [0] * args['hidden_size']
                    vector[-1 - VECTOR_PRE_X], vector[
                        -1 - VECTOR_PRE_Y] = point_pre[0], point_pre[1]
                    vector[-1 -
                           VECTOR_X], vector[-1 -
                    ...
                    if args['semantic_lane']:
                        vector[
                            -8] = 1 if lane_segment.has_traffic_control else -1
                    ...
                    point_pre_pre = (2 * point_pre[0] - point[0],
...
def preprocess(args: Dict, id2info: Dict, mapping: Dict):
    """
    ...
    for id in keys:
        ...
        for i, line in enumerate(info):
            ...
            if i > 0:
                ...
                vector = [
                    line_pre[X], line_pre[Y], x, y, line[TIMESTAMP],
                    line[OBJECT_TYPE] == 'AV', line[OBJECT_TYPE] == 'AGENT',
                    line[OBJECT_TYPE] == 'OTHERS',
                    len(polyline_spans), i
                ]
                vectors.append(get_pad_vector(vector, args['hidden_size']))
            line_pre = line
    ...
    t = len(vectors)
    mapping['map_start_polyline_idx'] = len(
        polyline_spans)  # every poly's start index in spans
    if args['use_map']:
        vectors, polyline_spans = get_sub_map(args,
                                              mapping['cent_x'],
                                              mapping['cent_y'],
                                              mapping['city_name'],
                                              vectors=vectors,
    ...
    if args['set_predict']:
        if 'test' in args[args['mode']]['data_dir'][0]:
            labels = [0.0 for _ in range(60)]
    ...
    mapping.update(
        dict(
            ...
            polyline_spans=[
                slice(each[0], each[1]) for each in polyline_spans
            ],
...
def argoverse_get_instance(lines, file_name, args):
    ...
    for id in id2info:
        info = id2info[id]
        for line in info:
            line[X], line[Y] = rotate(line[X] - mapping['cent_x'],
                                      line[Y] - mapping['cent_y'], angle)
        if 'scale' in mapping:
            scale = mapping['scale']
            line[X] *= scale
            line[Y] *= scale
...
class ArgoverseDataset(torch.utils.data.Dataset):
    """Argoverse Dataset class"""
    def __init__(self,
                 args: Dict,
                 mode: Optional[str] = 'train',
                 to_screen: bool = True):
        args['mode'] = mode
        data_dir = [args[mode]['data_dir']]
        self.ex_list = []
        if not os.path.isdir(args['temp_file_dir']):
            os.makedirs(args['temp_file_dir'])
        ...
        if args[mode]['reuse_temp_file']:
            ...
            pickle_file.close()
            logger.info(f"Reuse tmp argoverse {mode} data: {args['temp_file_dir']}")
        else:
            global am
            logger.info(f"Preprocess data in: {data_dir}")
            am = ArgoverseMap()
            if args['core_num'] >= 1:
                # TODO
                files = []
